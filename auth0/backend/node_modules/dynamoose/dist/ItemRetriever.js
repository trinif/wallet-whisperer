...
class ItemRetriever extends InternalPropertiesClass_1.InternalPropertiesClass {
    exec(callback) {
        ...
        const prepareForReturn = async (result) => {
            ...
            const buildItemFromModel = (model, item) => new model.Item(item, { "type": "fromDynamo" }).conformToSchema({ "customTypesDynamo": true, "checkExpiredItem": true, "saveUnknown": true, "modifiers": ["get"], "type": "fromDynamo", "mapAttributes": true });
            const array = (await Promise.all(result.Items.map(async (item) => {
    ...
    constructor(models, typeInformation, object) {
        super();
        let condition;
        try {
...
ItemRetriever.prototype.getRequest = async function () {
    const { models } = this.getInternalProperties(internalProperties).internalSettings;
    const table = models[0].getInternalProperties(internalProperties).table();
    const object = Object.assign(Object.assign({}, await this.getInternalProperties(internalProperties).settings.condition.getInternalProperties(internalProperties).requestObject(models, { "conditionString": "FilterExpression", "conditionStringType": "array" })), { "TableName": table.getInternalProperties(internalProperties).name });
    ...
    if (this.getInternalProperties(internalProperties).settings.startAt) {
        object.ExclusiveStartKey = Item_1.Item.isDynamoObject(this.getInternalProperties(internalProperties).settings.startAt) ? this.getInternalProperties(internalProperties).settings.startAt : models[0].Item.objectToDynamo(this.getInternalProperties(internalProperties).settings.startAt);
    }
    const allIndexes = await Promise.all(models.map((model) => model.getInternalProperties(internalProperties).getIndexes()));
    const mergedIndexes = utils_1.default.merge_objects.main({
        "combineMethod": utils_1.default.merge_objects.MergeObjectsCombineMethod.ArrayMergeNewArray,
    ...
    if (this.getInternalProperties(internalProperties).settings.index) {
        ...
        const indexSpec = utils_1.default.find_best_index(mergedIndexes, comparisonChart);
    ...
    if (this.getInternalProperties(internalProperties).internalSettings.typeInformation.type === "query") {
        const index = utils_1.default.array_flatten(Object.values(mergedIndexes)).find((index) => index.IndexName === object.IndexName) || mergedIndexes.TableIndex;
